...
export default function StoreCreditPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <h1 className="mobile-h1 mb-6 text-3xl font-bold text-foreground">Store credit</h1>
      <div className="bg-background border-2 border-primary rounded-lg p-8 text-center mb-8">
        <p className="text-sm text-foreground/70 mb-2">Available balance</p>
        <p className="text-5xl font-bold text-primary">{formatPrice(data?.balance || 0)}</p>
      </div>
      <h2 className="text-xl font-semibold mb-3">History</h2>
      {data && data.history.length > 0 ? (
        <div className="space-y-2">
          {data.history.map((entry, i) => (
            <div key={i} className="flex justify-between bg-background border border-foreground/15 rounded p-3">
              <div>
                <p className="capitalize text-sm font-medium">{entry.reason}</p>
                <p className="text-xs text-foreground/60">{new Date(entry.createdAt).toLocaleString()}</p>
              </div>
              <p className={`font-semibold ${entry.type === 'credit' ? 'text-primary' : 'text-foreground/78'}`}>
